...
class MandelbrotEosfinex extends EventEmitter {
  ...
  async getSignedTx (user, opts = {}) {
    const auth = user || await this.getAuth()

    const args = {}
    const { exchangeContract } = this.conf.eos
    const contract = opts.contract ? opts.contract : exchangeContract

    const action = contract === exchangeContract ? 'validate' : 'verify'

    let meta = null
    if (contract !== exchangeContract) {
      args.account = auth.account
    } else {
      meta = await this.requestChainMeta('priv')
    }
