...
class MandelbrotEosfinex extends EventEmitter {
  ...
  sendAuth (user, signed) {
    ...
    if (!this.tos) {
      throw new Error('TOS needs to be accepted')
    }
  ...
  async deposit (data) {
    const { eos } = this.conf
    const { exchangeContract, tokenContract } = eos
