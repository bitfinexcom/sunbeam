...
class MandelbrotEosfinex extends EventEmitter {
  constructor (client, opts = {}) {
    ...
    this.wallet = null
    if (opts.eos.auth.ual && client) {
      this.setClient({
        ...client,
        ual: opts.eos.auth.ual.user
      })
      this.wallet = new SunbeamUAL(opts.eos.auth.ual)
    } else if (client) {
  ...
  async place (order) {
    ...
    const o = new Order(order, { seskey1, seskey2, ccyMap })
