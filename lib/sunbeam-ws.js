...
class MandelbrotEosfinex extends EventEmitter {
  ...
  async deposit (payload) {
    const { exchangeContract, expireInSeconds } = this.conf.eos
    ...
    const txRes = await this.client.api.transact({ actions }, {
      blocksBehind: 3,
      broadcast: true,
    ...
    if (!txRes.transaction_id || !txRes.processed) {
      throw new Error('Transaction processing has failed')
    }
